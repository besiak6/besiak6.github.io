...
(function() {
    ...
    const ADDON_ID = "II"; // Zmieniono na Item Info (II)

    const styleSheet = document.createElement("style");
    styleSheet.type = "text/css";
    styleSheet.className = "ii-custom-styles";
    styleSheet.innerText = `
        .baddonz-ii-wnd { width: 180px; min-width: 180px; }
    ...
    function saveSettings() {
        ...
        const accKeys = ['windowOpacity', 'windowVisible', 'hideOpis', 'amountEssence', 'upgradeLevel', 'showSummaryLegend', 'showCommon', 'showUnique', 'showHeroic', 'showLegendary'];
        let accSettings = {};
    ...
    function waitForEngine() {
        return new Promise(resolve => {
            ...
            const checkExist = setInterval(() => {
                if (window.Engine && window.Engine.items && window.Engine.allInit) {
                    clearInterval(checkExist); resolve();
                }
            }, 100);
        });
    }
    ...
    function calculateCosts(level, artisanBonus, rarity) {
        ...
        let basePoints; let totalGoldCost;
    ...
    function shouldShowInfo(rarity) {
        ...
        switch (rarity) {
            case "common": return currentSettings.showCommon;
            case "unique": return currentSettings.showUnique;
            case "upgraded": return true; 
            case "heroic": return currentSettings.showHeroic;
            case "legendary": return currentSettings.showLegendary;
            default: return false;
        }
    }

    function removeCurrentAddonDataFromTip($target, tipId, originalTipHTML) {
        let cleanHTML = originalTipHTML;
        ALL_MARKERS.forEach(marker => {
            const regex = new RegExp(`<div class="item-tip-section ${marker}">[\\s\\S]*?<\\/div>`, 'g');
            cleanHTML = cleanHTML.replace(regex, '');
        });

        const essenceRegex = / \[\<span class="c_green c_essence_marker" style="font-weight: bold;">\d+<\/span>\]/;
        cleanHTML = cleanHTML.replace(essenceRegex, '');

        window.TIPS.allTips[tipId] = cleanHTML;
    }
    ...
    function addLegendInfoToTip() {
        ...
        removeCurrentAddonDataFromTip($target, tipId, currentTipContent);
        ...
        if (currentSettings.showSummaryLegend) {
            let summaryContent = ``;
            let upgradeIcon; let essenceIcon;
    ...
    function buildUI() {
        const bodyHtml = `
            <div class="baddonz-setting-row"><div class="baddonz-checkbox ii-hide-opis ${currentSettings.hideOpis ? 'active' : ''}"></div><span class="baddonz-text">Ukryj Opis Przedmiotu</span></div>
            <div class="baddonz-setting-row"><div class="baddonz-checkbox ii-amount-essence ${currentSettings.amountEssence ? 'active' : ''}"></div><span class="baddonz-text">Pokazuj ilość esencji z niszczenia</span></div>
            
            <hr style="width: 100%; border-color: #303030;">
            <div class="baddonz-setting-row"><div class="baddonz-checkbox ii-upgrade-level ${currentSettings.upgradeLevel ? 'active' : ''}"></div><span class="baddonz-text">Pokaż szczegółowe koszty poziomów</span></div>
            <div class="baddonz-setting-row"><div class="baddonz-checkbox ii-show-summary ${currentSettings.showSummaryLegend ? 'active' : ''}"></div><span class="baddonz-text">Pokaż ogólne podsumowanie ulepszenia</span></div>
            
            <hr style="width: 100%; border-color: #303030;">
            <span class="baddonz-text" style="padding-left:0; margin-bottom: 2px;">Dla jakich rzadkości pokazywać:</span>
            <div class="baddonz-setting-row"><div class="baddonz-checkbox ii-show-legendary ${currentSettings.showLegendary ? 'active' : ''}"></div><span class="baddonz-text">Legendarne</span></div>
            <div class="baddonz-setting-row"><div class="baddonz-checkbox ii-show-heroic ${currentSettings.showHeroic ? 'active' : ''}"></div><span class="baddonz-text">Heroiczne</span></div>
            <div class="baddonz-setting-row"><div class="baddonz-checkbox ii-show-unique ${currentSettings.showUnique ? 'active' : ''}"></div><span class="baddonz-text">Unikaty</span></div>
            <div class="baddonz-setting-row"><div class="baddonz-checkbox ii-show-common ${currentSettings.showCommon ? 'active' : ''}"></div><span class="baddonz-text">Zwykłe</span></div>
        `;
        ...
        bindCheckbox('.ii-hide-opis', 'hideOpis');
        bindCheckbox('.ii-amount-essence', 'amountEssence');
        bindCheckbox('.ii-upgrade-level', 'upgradeLevel');
        bindCheckbox('.ii-show-summary', 'showSummaryLegend');
        bindCheckbox('.ii-show-legendary', 'showLegendary');
        bindCheckbox('.ii-show-heroic', 'showHeroic');
        bindCheckbox('.ii-show-unique', 'showUnique');
        bindCheckbox('.ii-show-common', 'showCommon');
    }
